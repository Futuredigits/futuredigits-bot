    ...
    async def _reconcile_stars():
        try:
            ...
            while True:
                ...
                resp = await bot.session.make_request("getStarTransactions", payload)
                txs = resp.get("transactions") if isinstance(resp, dict) else (resp or [])
                if not txs:
                    break
                ...
                for tx in txs:
                    ...
                    m = re.match(r"premium:monthly:(\d+):", str(payload_str))
                    if not m:
                        continue
                    uid = int(m.group(1))
                    ...
                    if isinstance(exp, (int, float)):
                        ...
                        if isinstance(tx_date, (int, float)):
                            exp_ts = int(tx_date) + 2592000
                        else:
                            exp_ts = int(datetime.now(dt_timezone.utc).timestamp()) + 2592000
                ...
                offset = resp.get("next_offset") if isinstance(resp, dict) else None
                if not offset:
                    break

            logging.info(f"[stars] reconciled {seen} subscription tx(s)")
        except Exception as e:
            logging.warning(f"[stars] reconcile failed: {e}")
    
    _scheduler.add_job(job_daily,  CronTrigger(hour=8,  minute=0,  timezone=SCHED_TZ), id="daily_vibe_0800",       replace_existing=True)
